...
# * To convert to .exe file, run: pyinstaller --onefile --windowed --noconsole converter.py
...
class CurrencyConverter:  # Create class
    def __init__(self):  # Special method in Python class
        ...
        window.configure(bg="#FFFF00")  # Add background color to application window

        # Adding Label widgets to application window
        Label(
            window, font="Helvetica 12 bold", bg="#FFFF00", text="Amount to convert"
        ).grid(row=1, column=1, sticky=W)
        Label(
            window, font="Helvetica 12 bold", bg="#FFFF00", text="Conversion Rate"
        ).grid(row=2, column=1, sticky=W)
        Label(
            window, font="Helvetica 12 bold", bg="#FFFF00", text="Converted Amount"
